      ...
      ['Womit verbringst du den Abend?',
       /* ── Zwei Fehler in einer Zeile ──
          **`rangName(S.rang)` schrieb den eigenen Rang hin** — bei einem
          Capitaine also „drei Abende mehr als Capitaine". Gemeint ist der
          Rang, ab dem es die Zulage *nicht* gibt: der Fusilier.

          Und die Zahlwörter endeten bei zwei, während `abendeFuer()` seit der
          dritten und vierten Stufe bis **vier** gibt. Ein Général de division
          las „zwei Abende mehr", wo vier gemeint waren. */
       `Verbleibend ${L.abende} von ${abendeFuer(n)}${abendeFuer(n)>n.abende
          ? ` · ${['','ein Abend','zwei Abende','drei Abende','vier Abende'][abendeFuer(n)-n.abende] || (abendeFuer(n)-n.abende)+' Abende'} mehr als ein Fusilier`
          : ''}`],
      opt,
