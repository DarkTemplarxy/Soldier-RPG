...
function zeigeEreignis(e){
  ...
  const opt = e.optionen.map((o,i)=>`<button class="ord ${o.risk?'risk':''}" onclick="ereignisWaehlen(${i})">
...
function ereignisWaehlen(i){
  ...
  const w = p.erfolg ? o.erfolg : (o.misserfolg || o.erfolg);
  K.ereignis = null;
  ereignisWirkung(w);
  ...
  if(S.leben <= 0){
    kampfEnde(false, w.text + ' Du setzt dich hin, weil du nicht anders kannst, und stehst nicht wieder auf.');
    toetlich('Gefallen bei '+n.datum.split(' · ')[1]);
    zeigeTod(); return;
  }
  if(w.ende==='sieg' || K.feindMoral <= 0){ kampfEnde(true, w.text); return; }
  K.runde++;
  if(K.runde > n.runden){
    const knapp = K.feindMoral < n.feindMoral*0.35;
    kampfEnde(knapp, w.text + (knapp?' Und dann ist es plötzlich vorbei.':' Es wird dunkel, und nichts ist entschieden.'));
    return;
  }
  laufSichern();
  zeigeKampf(w.text);
